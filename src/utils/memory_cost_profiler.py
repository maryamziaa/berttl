...
def count_activation_size(net, args, input_size=(8, 128), require_backward=True, activation_bits=32):
	...
	#****input********
	x = {"input_ids": torch.ones(size=input_size, dtype=torch.long).to(args.device),
	"attention_mask": torch.ones(size=input_size,dtype=torch.long).to(args.device), 
	"labels": torch.zeros(size=(input_size[0],),dtype=torch.long).to(args.device)}
	if args.model_type != 'distilbert':
		x['token_type_ids'] = torch.zeros(size=input_size,dtype=torch.long).to(args.device) if args.model_type in ['bert', 'xlnet', 'mobilebert'] else None  # XLM, DistilBERT and RoBERTa don't use segment_ids
...
def profile_memory_cost(net, args, input_size=(8, 128), require_backward=True,
	...
	activation_size, _ = count_activation_size(net, args, input_size, require_backward, activation_bits)
