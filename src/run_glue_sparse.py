...
def train(args, train_dataset, model, tokenizer):
    ...
    if args.local_rank in [-1, 0]:
        out=args.output_dir
        writer_dir= out.split('/')[-1]
        tb_writer = SummaryWriter(('runs/'+ writer_dir))
...
class SparseIntermediate(nn.Module):
    def __init__(self, *args):
        super().__init__()
        hidden_act = "relu"
        self.dense = nn.Linear(args[0], args[1], bias=True)
        #self.sparse = sl.SparseLinear(args[0], args[1], sparsity= 0.5, dynamic=False, bias=True)
        #self.dense = BlockSparseLinear(args[0], args[1], density=0.1)
...
def main():
    ...
    parser.add_argument("--memory_cost", action='store_true', help="generate memory cost")
    ...
    for name, layer_module in model.named_modules():
        if 'ffn' in name:
            if isinstance(layer_module, nn.Linear):
                if prune_alg == "unstruct":
                   #prune.random_unstructured(layer_module, name="weight", amount=0.1)
                   prune.l1_unstructured(layer_module, name="weight", amount=0.1)
                   prune.remove(layer_module, name="weight")
                   layer_module._forward_pre_hooks
                   #print(layer_module.weight)
                if prune_alg == "struct":
                    prunable_layers += [name]


    '''
        from torchpruner.pruner import Pruner
        layers = list(model.fnn.children())
    ...
    #invalid_call()
    #pretrained_dict = {'dense.weight':main_dict['dense.weight']}

    tsf_module = model.mobilebert.encoder.layer
    for i, layer_module in enumerate(tsf_module):
        ...
        for ffn_layer_ind in range(0,3):
            ...
            if ffn_module[ffn_layer_ind].intermediate:
                ffn_module[ffn_layer_ind].intermediate = SparseIntermediate(128, 512)
            if ffn_module[ffn_layer_ind].output:
                ffn_module[ffn_layer_ind].output.dense = SparseIntermediate(512, 128)
    #main_dict = model.state_dict()
    #print('***main_dict***', main_dict)
    #invalid_call()
    #print('****model****', model)
    ...
    if args.memory_cost:
        ...
        input_size = (args.per_gpu_train_batch_size,args.max_seq_length)
        memory_cost, detailed_info = profile_memory_cost(
            model, args, input_size, require_backward, activation_bits=32, trainable_param_bits=32,
            frozen_param_bits=8, batch_size=args.per_gpu_train_batch_size
            )
        net_info = {
